...
const Problem = () => {
  ...
  const getSubmissions = async () => {
    ...
    // console.log(json.submissions.statuses);
  ...
  const ShowSubmissions = (props) => {
    ...
    return (
      <div>
        {submissions.length == 0? (<p className="status">No problems yet</p>):submissions.map((status) => {
            return (
...
function LanguageSelector(props) {
  ...
  const submitProblem = async () => {
    const response = await fetch(
      "http://localhost:3000/submission/" + props.problemId,
      {
        method: "POST",
        headers: {
          authorization: localStorage.getItem("token"),
